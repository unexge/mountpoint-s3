...
macro_rules! err {
    ...
    ($errno:expr, source:$source:expr, $level:expr, metadata:$metadata:expr, $message:literal) => {
        err!($errno, __source:Some(::std::sync::Arc::new(::anyhow::Error::new($source))), $level, $metadata, $message, )
    };
    ...
    ($errno:expr, source:$source:expr, $level:expr, $message:literal, $($args:tt)*) => {
        err!($errno, __source:Some(::std::sync::Arc::new(::anyhow::Error::new($source))), $level, Default::default(), $message, $($args)*)
    };
    ($errno:expr, source:$source:expr, $level:expr, $message:literal) => {
        err!($errno, __source:Some(::std::sync::Arc::new(::anyhow::Error::new($source))), $level, Default::default(), $message,)
    };
    ($errno:expr, source:$source:expr, $message:literal, $($args:tt)*) => {
        err!($errno, __source:Some(::std::sync::Arc::new(::anyhow::Error::new($source))), ::tracing::Level::WARN, Default::default(), $message, $($args)*)
    };
    ($errno:expr, source:$source:expr, $message:literal) => {
        err!($errno, __source:Some(::std::sync::Arc::new(::anyhow::Error::new($source))), ::tracing::Level::WARN, Default::default(), $message,)
    };
...
impl From<InodeError> for Error {
    fn from(err: InodeError) -> Self {
        ...
        Error {
            ...
            source: Some(Arc::new(anyhow::anyhow!(err))),
